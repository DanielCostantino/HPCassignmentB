...
// mpirun -np 1 density.o
// mpirun -np 1 density.o 2 2 input.bin
// mpirun -np 2 density.o 2 2 input.bin
// mpirun -np 2 density.o 2 2

// mpirun --mca pml ob1 --mca btl tcp,self -np 4 density.o 2 2 input.bin  //pml -> ob1; btl -> tcp
// mpirun --mca pml ucx --mca btl tcp,self --map-by node -np 2 density.o 2 2 //pml -> ucx; btl -> tcp
...
int main(int argc, char *argv[])
{
    ...
    if(argc > 1){
	   ...
	   printf ("# Number of tasks= %d with rank= %d running on %s\n", numprocs, rank, hostname);
	    		...
	    		if(fptr = fopen(argv[3],"rb")){
			    	...
			    	for(int i = 0; i < nPart; i++)
					{
				    	...
				    	*(pd.partsPositions+i*3+2) = coords[2];
				    	printf("x_%d: %f\ty_%d: %f\tz_%d: %f\n", i,*(pd.partsPositions+i*3), i,*(pd.partsPositions+i*3+1), i, *(pd.partsPositions+i*3+2));
			    	}	
				    ...
				    // WRITING in the file:
						...
						pd.numParts = 1 + (rand()%g.numGrid_points); //generated randomly between 0 and N
					    printf("Np:\t%d\n\n", pd.numParts); 	
		  ...
		  //Sending to each MPIprocess the particles distribution:
			...
			if(rank == ROOT){
				#pragma omp parallel num_threads(8) proc_bind(close)
				{
				   #pragma omp master
				    {
					    char *proc_bind_names[] = { "false (no binding)", "true", "master", "close", "spread" };					  					      
			...
			{			
				...
				printf ( "\n\tRunning time of the ROOT processor -> %10.8f\n\n", t);
			}
